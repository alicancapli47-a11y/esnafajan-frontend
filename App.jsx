...
export default function App() {
  ...
  if (urlSession) return <ResultScreen sessionId={urlSession} />
  return <UploadScreen onSession={(id) => { window.location.href = '/app?session_id=' + id }} />
}

function AuthScreen({ plan }) {
  ...
  const submit = () => {
    setError('')
    if (!email || !pass) return setError('Email ve şifre zorunlu')
    if (mode === 'register' && !name) return setError('Ad soyad zorunlu')
    setLoad(true)
    setTimeout(() => { window.location.href = '/siparis' }, 500)
  }

  return (
    <div style={s.page}>
      <Header />
      <div style={{ ...s.main, maxWidth: 420, paddingTop: '3rem' }}>
        <div style={s.card}>
          <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 6, color: '#0d3d26' }}>
            {mode === 'register' ? 'Ücretsiz Üye Ol' : 'Giriş Yap'}
          </h2>
          <p style={{ fontSize: 13, color: '#888', marginBottom: '1.5rem' }}>
            {mode === 'register' ? 'Kart bilgisi gerekmez.' : 'Hesabına giriş yap.'}
          </p>
          {mode === 'register' && (
            <input style={s.input} placeholder="Ad Soyad" value={name} onChange={e => setName(e.target.value)} />
          )}
          <input style={s.input} placeholder="E-posta" type="email" value={email} onChange={e => setEmail(e.target.value)} />
          <input style={s.input} placeholder="Şifre" type="password" value={pass} onChange={e => setPass(e.target.value)} />
          {error && <div style={s.errorBox}>{error}</div>}
          <button style={{ ...s.submitBtn, opacity: loading ? 0.7 : 1, marginTop: 8 }} onClick={submit} disabled={loading}>
            {loading ? 'Yükleniyor...' : mode === 'register' ? 'Üye Ol →' : 'Giriş Yap →'}
          </button>
          <p style={{ fontSize: 13, color: '#888', textAlign: 'center', marginTop: '1rem' }}>
            {mode === 'register' ? 'Zaten üye misin? ' : 'Hesabın yok mu? '}
            <span style={{ color: '#1a7f5a', cursor: 'pointer', fontWeight: 600 }}
...
const s = {
  ...
  input:     { width: '100%', border: '0.5px solid #e5e5e5', borderRadius: 8, padding: '10px 14px', fontSize: 14, fontFamily: 'inherit', outline: 'none', marginBottom: 10, boxSizing: 'border-box' },
